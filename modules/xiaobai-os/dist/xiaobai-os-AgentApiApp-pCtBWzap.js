/* eslint-disable */
import { E as Dt, G as Bt, H as jt, J as lt, K as xe, M as we, Q as qe, T as Ft, X as Kt, b as zt, f as Ie, g as _e, h as Ht, k as Gt, p as w, s as Jt } from "./xiaobai-os-runtime-dom.esm-bundler-BcM9c-Z9.js";
import { t as Vt } from "./xiaobai-os-host-request-headers-DX4NEJeo.js";
...
function le(t = {}) {
  const n = t && typeof t == "object" ? t : {}, s = Qt(n.effort), i = Zt(n.budgetTokens);
  return {
    mode: Xt(n.mode),
    ...s ? { effort: s } : {},
...
var bt = "openai-compatible", $e = "默认", vt = "default", en = "deny", G = 32e3, tn = Object.freeze([{
  value: "default",
  label: "默认权限"
}, {
  value: "full",
...
function dn(t = {}, n, s) {
  ...
  return i(t?.delegateConfig?.tavilyBaseUrl) ? F(t.delegateConfig.tavilyBaseUrl) : Wt;
}
function un(t = {}, n, s) {
  return {
...
function ke(t = {}) {
  const n = P(t.currentPresetName || t.presetDraftName || "默认"), s = an(t, n), i = sn(s, t.currentPresetName), o = rn(s, t.delegatePresetName, i), d = s[i] || _(), u = s[o] || d, g = xt(t.delegateConfig, u), m = on(t, s, i, o), v = un(t, n, i);
  return {
...
var ce = "openai", ht = "claude", Tt = "makersuite", mn = "/api/backends/chat-completions/status", fn = "/api/backends/chat-completions/generate", Pt = Object.freeze({
...
function de(t = "", n = "", s = null) {
  ...
  return kn(t) || String(t || n || "").trim();
}
function En(t = {}, n = ce) {
  const s = vn(t.baseUrl, n), i = String(t.apiKey || "").trim(), o = Pt[n] || "", d = s || (i ? o : ""), u = { chat_completion_source: n || "openai" };
...
async function Nn(t = {}, n = ce, s = {}, i = {}) {
  const o = await Fe(i.fetch)(mn, {
    method: "POST",
    headers: await Mt(i.requestHeadersProvider),
    body: JSON.stringify(Cn(t, n)),
    signal: s.signal
  }), d = await o.text();
  let u = null;
  try {
...
async function Ke(t = {}, n = ce, s = {}) {
  return await Nn(t, n, s, { requestHeadersProvider: fe });
}
...
async function qn(t = {}, n = {}) {
  return await wn(t, n, { requestHeadersProvider: fe });
}
async function In(t = {}, n, s = {}, i = {}) {
...
async function _n(t = {}, n, s = {}) {
  return await In(t, n, s, { requestHeadersProvider: fe });
}
var ka = Object.freeze([
  ...
  buildHostChatCompletionGenerateRequest: Sn,
...
var ze = At({
  ...
  "xhigh",
  "max"
], "medium", { temperatureOmitModes: He }), Un = K("kimi-k3", [
  "inherit",
  "on",
  "off"
], [
  ...
  "high",
  "max"
], "high", { temperatureOmitModes: be }), $n = K("openai-compatible-gemini-latest", [
  "inherit",
  "on",
  "off"
], [
  ...
  "xhigh",
  "max"
], "high", { temperatureOmitModes: be }), Bn = K("openai-compatible-default", [
  "inherit",
  "on",
  "off"
], [
  ...
  "medium",
  "high"
], "medium", { temperatureOmitModes: be }), jn = K("anthropic-adaptive", [
  "inherit",
  "on",
  "off"
], [
  ...
  "xhigh",
  "max"
], "high", { temperatureOmitModes: He }), Fn = K("sillytavern-claude-adaptive", [
  "inherit",
  "on",
  "off"
], [
  ...
  "high",
  "max"
], "high", { temperatureOmitModes: He }), Kn = K("google-gemini-3-flash", ["inherit", "on"], [
  "minimal",
  "low",
  "medium",
  "high"
], "high"), zn = K("sillytavern-google-3-flash", ["inherit", "on"], [
  "min",
  "low",
  "medium",
  "high"
...
function he(t = {}, n = {}) {
  const s = Ge(t), i = le(n), o = n?.output === "show" || n?.output === "hide" ? n.output : null, d = Vn({
    ...i,
...
function ge(t = []) {
  const n = [...new Set(t.filter(Boolean).map((o) => String(o).trim()).filter(Boolean))], s = Yn.chat, i = n.filter((o) => {
...
async function la(t, n = {}) {
  ...
  if (s === "sillytavern-claude") return ge(await oa(t, n));
  if (Qn(s)) return ge(await Ke(t, ea(s), { signal: n.signal }));
...
function ua(t = {}) {
  const { state: n, render: s, showToast: i, createRequestId: o = (e = "req") => `${e}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`, saveConfig: d, pullModels: u = la, describeError: g = da, getRuntimeSummaryText: m } = t;
  function v() {
  ...
  function B(e, r = "main") {
    ...
    H && (j(H, Gn(T)), H.value = k), ne && (ne.textContent = T.unsupportedReason || `能力配置：${T.profileId}`), se && (j(se, Jn(T)), se.value = C), ae && (ae.style.display = k === "on" && T.intensity.kind === "effort" ? "" : "none"), Z && T.intensity.kind === "budget" && (Z.min = T.intensity.allowAuto ? "-1" : String(T.intensity.min), Z.max = String(T.intensity.max), Z.value = String(Q)), re && (re.style.display = k === "on" && T.intensity.kind === "budget" ? "" : "none");
  }
  function z(e) {
  ...
  function We(e) {
    ...
    j(re, rt), re.value = r.currentPresetName || n.config.currentPresetName || "默认", Ce && (j(Ce, rt), Ce.value = q(r.delegatePresetName, r.currentPresetName)), Z.value = r.presetDraftName || r.currentPresetName || "默认", S && (S.value = a), M && (M.value = r.baseUrl || ""), A && (A.value = r.model || ""), T && (T.value = r.apiKey || ""), se && (se.value = String(N(r.maxTokens))), V && (V.value = String(U(r.temperature, 1))), k && (k.checked = !!(r.sendTemperature ?? !0)), nt && (nt.value = r.tavilyApiKey || ""), C && (C.style.display = ie(a) ? "" : "none"), Q && (j(Q, pt), Q.value = r.toolMode || "native"), H && (j(H, tn), H.value = oe(r.permissionMode)), ne && (j(ne, nn), ne.value = Y(r.jsApiPermission)), B(e), ae && (j(ae, l.map((W) => ({
      value: W,
      label: W
...
function ga(t = {}) {
  const { configSave: n = {}, runtimeText: s = "", inlineToastText: i = "", showInlineToast: o = !0, showAssistantPermissions: d = !0, showDelegateSettings: u = !0, showTavilySettings: g = !0, activePage: m = "main", delegatePresetHint: v = "DelegateRun 分身会使用这里的独立 API 配置；可以和主助手使用不同 Provider、Base URL、模型和 Tool 调用格式。", isBusy: p = !1, canDeletePreset: L = !0, configLoadError: D = "" } = t, R = String(D || "").trim(), O = pa(n), q = ca(n), X = p || R || String(n?.status || "") === "saving" ? "disabled" : "", ee = p || !L ? "disabled" : "", te = m === "delegate" ? "delegate" : "main", $ = te === "main", y = te === "delegate", J = d ? `
            <label>
                <span>斜杠命令权限</span>
...
var ma = { class: "agent-api-app" }, fa = { class: "agent-api-scroll" }, ba = { class: "agent-api-content" }, va = {
...
}, xa = { "aria-live": "polite" }, ha = ["disabled"], ft = 13e4, Ta = /* @__PURE__ */ zt({
  __name: "AgentApiApp",
  props: {
    bridge: {},
    initialState: {}
  ...
  setup(t) {
    ...
    function J(b) {
      i.value = structuredClone(b), b.status === "ready" && b.config && (p.config = ke(b.config), p.configDraft = null, p.configDirty = !1, p.configFormSyncPending = !0), Ft(y);
    }
    async function f() {
      const b = o.value;
    ...
    }), (b, h) => (we(), _e("main", ma, [w("div", fa, [w("div", ba, [
      h[2] || (h[2] = w("header", { class: "agent-api-header" }, [w("h1", null, "Agent API 配置"), w("p", null, "共享 Agent 主预设")], -1)),
      i.value.status === "loading" ? (we(), _e("section", va, " 正在读取配置 ")) : i.value.status === "error" ? (we(), _e("section", ya, [w("div", null, [h[1] || (h[1] = w("strong", null, "配置暂时无法读取", -1)), w("span", null, qe(i.value.message), 1)]), w("button", {
        type: "button",
        onClick: h[0] || (h[0] = (E) => ee())
      }, "重新读取")])) : Ht("", !0),
      jt(w("section", Sa, [w("div", {
        ref_key: "panelRoot",
        ref: o
      }, null, 512), w("div", { class: Kt(["agent-api-connection", `is-${d.value}`]) }, [w("p", xa, qe(u.value), 1), w("button", {
        type: "button",
        disabled: !L.value || R.value,
        onClick: f
      }, qe(R.value ? "测试中…" : "测试当前连接"), 9, ha)], 2)], 512), [[Jt, L.value]])
    ])])]));
  }
}), Ca = Ta;
export {
