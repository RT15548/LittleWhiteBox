...
export interface DicePartition { schemaVersion: 1; actionChecksEnabled: boolean }
...
function parseDicePartition(value: unknown): DicePartition {
    ...
    if (input.schemaVersion !== 1 || typeof input.actionChecksEnabled !== 'boolean' || Object.keys(input).length !== 2) {
        throw new TypeError('dice_partition_invalid');
    }
    return { schemaVersion: 1, actionChecksEnabled: input.actionChecksEnabled };
}
...
export const DICE_PARTITION: PartitionRegistration<DicePartition> = Object.freeze({
    ...
    createInitial: () => ({ schemaVersion: 1 as const, actionChecksEnabled: false }),
});
